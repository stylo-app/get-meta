...
import { map, switchMap } from '@polkadot/x-rxjs/operators'; // structs need to be in order
...
const definitions = {
  ...
  derives: {
    ...
    balances: {
      account: (instanceId, api) => memo(instanceId, address => api.derive.accounts.accountId(address).pipe(switchMap(accountId => accountId ? combineLatest([of(accountId), api.queryMulti([[api.query.system.account, accountId]]).pipe(map(raw => {
        if (raw.length < 1) {
      ...
      }))]) : of([api.registry.createType('AccountId'), [api.registry.createType('Balance'), api.registry.createType('Balance'), api.registry.createType('Balance'), api.registry.createType('Index')]])), map(([accountId, [freeBalance, reservedBalance, vestingLocked, accountNonce]]) => ({
        accountId,
        accountNonce,
        additional: [],
        freeBalance,
